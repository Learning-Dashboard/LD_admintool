...
public class TaigaValidationService {
    ...
    public ValidationResult validateProjectBySlug(String projectSlug) {
        ...
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful()) {
                ...
                System.out.println("❌ Projecte Taiga '" + projectSlug + "' no accessible (status: " + response.getStatusCode() + ")");
            }
            ...
            result.addError("No tens autorització per accedir al projecte Taiga '" + projectSlug + "' (comprova el token de Taiga)");
    ...
    public ValidationResult validateUsersInProject(String projectSlug, List<String> usernames) {
        ...
        try {
                ...
                if (membersNode != null && membersNode.isArray()) {
                    ...
                    for (JsonNode memberNode : membersNode) {
                        ...
                        if (usernameNode != null) {
                            projectMembers.add(usernameNode.asText().toLowerCase());
                        }
                    }
                    ...
                    for (String username : usernames) {
                        ...
                        if (projectMembers.contains(username.toLowerCase())) {
                            System.out.println("✅ Usuari '" + username + "' és membre del projecte Taiga '" + projectSlug + "'");
                        } else {
                            result.addError("L'usuari '" + username + "' NO és membre del projecte Taiga '" + projectSlug + "'");
                            System.out.println("❌ Usuari '" + username + "' NO trobat al projecte Taiga");
                        }
                    }
                } else {
                    result.addWarning("No es poden validar els membres del projecte Taiga '" + projectSlug + "' (membres no disponibles)");
            ...
            result.addWarning("El projecte Taiga '" + projectSlug + "' és privat o no tens permisos per veure els membres");
            System.out.println("⚠️ Projecte Taiga '" + projectSlug + "' és privat (403)");
        } catch (Exception e) {
